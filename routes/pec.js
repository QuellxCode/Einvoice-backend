...
router.post("/", auth, async (req, res) => {
  ...
  if (pec) return res.status(400).send("Pec Category already registered.");
...
router.get("/", auth, async (req, res) => {
  ...
  if (!pec) return res.status(400).send("No pec created.");
  res.json({
    pec
  });
});
...
router.patch("/:id", auth, async (req, res) => {
  ...
  if (!pec) return res.status(400).send("This Pec doesnot exists");
