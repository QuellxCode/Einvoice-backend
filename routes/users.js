...
router.post("/", auth, async (req, res) => {
  ...
  if (employee || cnic) return res.status(400).json({msg:"User already registered."});
  const hash = await bcrypt.hash(req.body.password, 10);
...
router.get("/", auth, async (req, res) => {
  ...
  if (!employee) return res.status(400).json({msg:"No employee created."});
  res.json({
    employee,
  });
});
...
router.patch("/:id", auth, async (req, res) => {
  ...
  if (!employee) return res.status(400).json({msg:"This User doesnot exists"});
